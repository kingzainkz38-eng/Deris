  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        {/* Left sidebar: photo, rating, contact */}
        <aside className="lg:col-span-1">
          <div className="flex flex-col items-center gap-3 rounded-2xl border border-neutral-200 bg-white p-8 text-center lg:sticky lg:top-24">
            {user.avatar_key ? (
              ...
              <div className="flex h-28 w-28 shrink-0 items-center justify-center rounded-full bg-[var(--brand-green)] text-4xl font-bold text-white">
                {user.name.charAt(0).toUpperCase()}
              </div>
            )}

            <h1 className="mt-1 text-2xl font-bold text-neutral-900">{user.name}</h1>
            ...
            {user.bio && <p className="text-sm text-neutral-700">{user.bio}</p>}

            {user.phone && (
              <a
                href={`tel:${user.phone}`}
                className="mt-2 w-full rounded-full bg-[var(--brand-green)] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[var(--brand-green-dark)]"
              >
                📞 {user.phone}
              </a>
            )}
            ...
            {isOwner && (
              <Link
                ...
                className="w-full rounded-full border border-neutral-300 px-4 py-2.5 text-sm font-semibold text-neutral-600 hover:bg-neutral-50"
              >
                Edit profile
              </Link>
            )}
        ...
        {/* Middle/main: listings, previous work, feedback */}
        <div className="lg:col-span-2">
          <h2 className="text-xl font-bold text-neutral-900">{t("profile_listings_heading")}</h2>
